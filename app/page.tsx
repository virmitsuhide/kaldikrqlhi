...
import AgendaBulanIni from '@/components/AgendaBulanIni'
...
export default function HomePage() {
  ...
  const fetchEvents = useCallback(async (y: number) => {
    ...
    const { data } = await supabase
      .from('events').select('*').eq('year', y).order('date', { ascending:true })
    setEvents(data ?? [])
    setLoading(false)
  }, [])
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 space-y-5">
        ...
        <div className="relative overflow-hidden rounded-3xl
                        ...
                        px-6 py-8 text-white shadow-xl">
          ...
          <div className="relative">
            ...
            <h1 className="font-display font-extrabold text-2xl md:text-3xl mb-1">
              Kalender Akademik RQ LHI
            </h1>
        ...
        {/* Agenda Bulan Ini — hanya tampil di tahun saat ini */}
        {isCurrentYear && !loading && (
          <AgendaBulanIni events={filtered} year={year} />
        )}
